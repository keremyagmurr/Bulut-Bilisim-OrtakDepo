...
const QUESTIONS = [
  ...
  { category: "Satranç", difficulty: "Kolay", type: "chess",
    ...
    options: ["Re8#", "Re7", "Rf1", "Rg1"], answer: 0 },
  { category: "Satranç", difficulty: "Orta", type: "chess",
    ...
    options: ["Nxc6 (Çatal!)", "Nf3", "d4", "Bc4"], answer: 0 },
  { category: "Satranç", difficulty: "Kolay", type: "chess",
  ...
  { category: "Satranç", difficulty: "Zor", type: "chess",
    ...
    options: ["Bxf7+ (Çatal!)", "O-O", "Bg5", "a3"], answer: 0 },
  ...
  { category: "Fark Bul", difficulty: "Kolay", type: "spotdiff",
    ...
    grid1: ["🍎","🍊","🍋","🍇","🍉","🍓","🍑","🍒","🫐"],
    grid2: ["🍎","🍊","🍋","🍇","🍉","🍓","🍑","🍌","🫐"],
    options: ["Satır 3 Sütun 2 (🍒→🍌)", "Satır 1 Sütun 3", "Satır 2 Sütun 1", "Satır 3 Sütun 3"], answer: 0 },
  { category: "Fark Bul", difficulty: "Kolay", type: "spotdiff",
    question: "Hangi emoji değişmiş?",
    grid1: ["⭐","🌙","☀️","🌈","⚡","❄️","🔥","💧","🌊"],
    grid2: ["⭐","🌙","☀️","🌈","🌟","❄️","🔥","💧","🌊"],
    options: ["Satır 2 Sütun 2 (⚡→🌟)", "Satır 1 Sütun 1", "Satır 3 Sütun 1", "Satır 1 Sütun 3"], answer: 0 },
  { category: "Fark Bul", difficulty: "Orta", type: "spotdiff",
    question: "Dikkatli bakın! Hangi hücre farklı?",
    grid1: ["🐶","🐱","🐭","🐹","🐰","🦊","🐻","🐼","🐨","🐯","🦁","🐮","🐷","🐸","🐵","🙈"],
    grid2: ["🐶","🐱","🐭","🐹","🐰","🦊","🐻","🐼","🐨","🐯","🦁","🐮","🐷","🐲","🐵","🙈"],
    options: ["Satır 4 Sütun 2 (🐸→🐲)", "Satır 2 Sütun 3", "Satır 3 Sütun 1", "Satır 1 Sütun 4"], answer: 0 },
  { category: "Fark Bul", difficulty: "Orta", type: "spotdiff",
    question: "Bir şey değişti! Hangisi?",
    grid1: ["🏠","🏢","🏥","🏫","🏪","🏭","🏗️","🏛️","🏟️"],
    grid2: ["🏠","🏢","🏥","🏫","🏪","🏭","🏗️","⛪","🏟️"],
    options: ["Satır 3 Sütun 2 (🏛️→⛪)", "Satır 1 Sütun 2", "Satır 2 Sütun 3", "Satır 3 Sütun 3"], answer: 0 },
  { category: "Fark Bul", difficulty: "Zor", type: "spotdiff",
    question: "25 emojide TEK fark! Bulabilir misiniz?",
    grid1: ["🔴","🟠","🟡","🟢","🔵","🟣","⚫","⚪","🟤","🔴","🟠","🟡","🟢","🔵","🟣","⚫","⚪","🟤","🔴","🟠","🟡","🟢","🔵","🟣","⚫"],
    grid2: ["🔴","🟠","🟡","🟢","🔵","🟣","⚫","⚪","🟤","🔴","🟠","🟡","🟢","🔵","🟣","⚫","⚪","🟤","🔴","🟠","🟡","🟢","🟤","🟣","⚫"],
    options: ["Satır 5 Sütun 3 (🔵→🟤)", "Satır 2 Sütun 4", "Satır 3 Sütun 1", "Satır 4 Sütun 5"], answer: 0 },
];
...
io.on('connection', (socket) => {
  ...
  socket.on('swapPlayer', ({ playerId, fromTeamId, toTeamId }) => {
    ...
    // Dinamik kapasite hesapla
    let maxCap;
    if (game.teamMode === 'solo') {
      maxCap = 1;
    } else if (game.teamMode === 'free') {
      maxCap = 999; // Serbest modda sınır yok
    } else {
      const currentMaxSize = Math.max(...Object.values(game.teams).map(t => t.members.length));
      maxCap = Math.max(currentMaxSize, game.teamMode === 'trio' ? 3 : (game.teamMode === 'pairs' ? 2 : 2));
    }
    if (toTeam.members.length >= maxCap) {
      return socket.emit('error', { message: `Bu modda bir takımda en fazla ${maxCap} kişi olabilir!` });
    }
  ...
  // ── OYUNCU TAŞI (Manuel Seçim İle) ──
  socket.on('movePlayer', ({ playerId, toTeamId }) => {
    if (socket.id !== game.host || game.phase !== 'teams') return;
    ...
    io.emit('gameState', getPublicState());
    console.log(`🔄 (Manuel) ${player.name}: ${fromTeamId} → ${toTeamId}`);
  });
